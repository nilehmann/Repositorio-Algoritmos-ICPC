...
public class ConvexHull {
	...
	static ArrayList<Point> convexHull(ArrayList<Point> P){
		...
		int k = 0;
		for(Point p : P){
			while( (k=H.size()) >= 2 && cross(H.get(k-2), H.get(k-1), p) <= 0) 
				H.remove(k-1);
			H.add(p);
		}
		for(int i = P.size() - 2, t = H.size()+1; i >= 0; i--){
			Point p = P.get(i);
			while( (k=H.size()) >= t && cross(H.get(k-2), H.get(k-1), p) <= 0)
				H.remove(k-1);
			H.add(p);
		}
	...
	/**
	 * Asume vertices en counter-clockwise. Polígono convexo.
	 * Se debe repetir el primer punto.
	 */
	...
	/**
	 * Producto cruz entre oa y ob. Si oab hace un giro
	 * hacia la izquierda devuelve mayor que cero, si lo 
	 * hace a la derecha menor que cero. Cero si son colineales
	 */
